...
const Campaign = ({ user, fetchCampaignInfo, startCampaignLevel, history }) => {
  const [belt, setBelt] = useState(null);
  const [progress, setProgress] = useState(null);

  useEffect(() => {
    fetchCampaignData(user);
  }, [user]);

  const fetchCampaignData = (user) => {
    if (user) {
      fetchCampaignInfo(user.userId, (data) => {
        setBelt(Math.min(MAX_BELT, getCurrentBelt(data.progress)));
  ...
  const getCurrentBelt = (progress) => {
    return Object.keys(progress.beltsCompleted).length + 1;
  };

  const chooseBelt = (belt) => {
    setBelt(belt);
  };

  const renderCampaignNoUser = () => {
    const belt = 1;
    const progress = { beltsCompleted: {}, levelsCompleted: {} };

    return (
      <div className="campaign-no-user">
        <div className="campaign-wrapper">
          <div className="campaign-title">{BELTS[belt]} Belt Campaign</div>
          <div className="campaign-levels-wrapper">
            {renderCampaignBeltLevels(belt, progress, true)}
          </div>
        ...
        >
          <div className="campaign-no-user-modal-content">
            ...
            <div className="campaign-no-user-title campaign-no-user-title-last">
              become a Chinchilla Chess master.{" "}
              <a href="/login?next=/campaign">Log in</a> to begin!
            </div>
            <div
  ...
  const renderCampaignBelts = (progress) => {
    ...
    return (
      <div className="campaign-belts">
        <div className="campaign-belts-row">
          {BELTS.map((_, beltIdx) => {
            return beltIdx > 0 ? renderLargeBelt(beltIdx, currentBelt) : null;
          })}
        </div>
      </div>
    );
  ...
  const renderLargeBelt = (beltIdx, currentBelt) => {
    ...
    if (beltIdx < currentBelt) {
      ...
      return (
        <Tooltip
          ...
          key={`belt-${beltIdx}`}
        >
          <div className="campaign-belt campaign-belt-locked" key={beltIdx}>
            <img src={`/static/belt-${beltName.toLowerCase()}.png`} />
          </div>
        </Tooltip>
      );
  ...
  const renderCampaignBeltLevels = (belt, progress, isActive) => {
    ...
    const levelClasses = [0, 1, 2, 3, 4, 5, 6, 7, 8].map((idx) => {
      ...
      if (
        levelOffset + idx === 0 ||
        progress.levelsCompleted[levelOffset + idx - 1]
      ) {
    ...
    return (
      ...
      >
        <div
          className={`campaign-levels ${
            isActive ? "campaign-levels-active" : ""
          }`}
        >
          <div className="campaign-levels-row">
            <div className="campaign-head" />
            {renderLevel(levelOffset + 0, levelClasses[0])}
            <div className={`campaign-path ${pathClasses[0]}`} />
            {renderLevel(levelOffset + 1, levelClasses[1])}
            <div className={`campaign-path ${pathClasses[1]}`} />
            {renderLevel(levelOffset + 2, levelClasses[2])}
            <div className="campaign-tail" />
          </div>
          <div className="campaign-levels-row">
            <div className="campaign-head" />
            {renderLevel(levelOffset + 5, levelClasses[5])}
            <div className={`campaign-path ${pathClasses[4]}`} />
            {renderLevel(levelOffset + 4, levelClasses[4])}
            <div className={`campaign-path ${pathClasses[3]}`} />
            {renderLevel(levelOffset + 3, levelClasses[3])}
            <div
              className={`campaign-tail campaign-tail-path ${pathClasses[2]}`}
            />
          </div>
          <div className="campaign-levels-row">
            <div
              className={`campaign-head campaign-head-path ${pathClasses[5]}`}
            />
            {renderLevel(levelOffset + 6, levelClasses[6])}
            <div className={`campaign-path ${pathClasses[6]}`} />
            {renderLevel(levelOffset + 7, levelClasses[7])}
  ...
  const renderLevel = (level, levelClass) => {
    return (
      <Tooltip
          ...
          <div className="campaign-levels-wrapper">
            {BELTS.map((beltName, beltIdx) => {
              if (beltIdx > 0 && beltIdx <= MAX_BELT) {
                return renderCampaignBeltLevels(
                  beltIdx,
                  progress,
                  belt === beltIdx
                );
...
//     const belt = 1,
//       progress = { beltsCompleted: {}, levelsCompleted: {} };
...
//           {BELTS.map((_, beltIdx) => {
//             return beltIdx > 0
//               ? this.renderLargeBelt(beltIdx, currentBelt)
//               : null;
//           })}
//         </div>
//       </div>
