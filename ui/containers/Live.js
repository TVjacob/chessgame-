...
export default function Live({ user, knownUsers, getLiveInfo, getRequest }) {
  ...
  const renderLiveGames = (activeGames) => {
    return (
      <div className="live-games">
        ...
        {activeGames && activeGames.length > 0 ? (
              ...
              {activeGames.map((g, index) => {
                ...
                return (
                  <tr className="live-games-row" key={index}>
                    ...
                    {renderGamePlayers(gameInfo.players)}
          ...
          <div className="live-games-empty">No games are currently live.</div>
        )}
      </div>
    );
  };

  const renderGamePlayers = (players) => {
  ...
  return (
    <div className="live">
      {fetching ? (
        ...
        <div className="live-content">{renderLiveGames(activeGames)}</div>
      )}
    </div>
  );
}
