...
const Replay = ({
  ...
  const [musicVolume, setMusicVolume] = useState(
    window.localStorage.musicVolume || 0
  );
  const [soundVolume, setSoundVolume] = useState(
    window.localStorage.soundVolume || 0
  );
  const [music, setMusic] = useState(null);
  const [captureSounds, setCaptureSounds] = useState(Array(8).fill(null));
  ...
  useEffect(() => {
    ...
    if (gameId) {
      ...
      startReplay(historyId, (data) => {
        history.push(`/replay/${historyId}?gameId=${data.gameId}`);
        initGameState(data.gameId);
      });
    }

    return () => clearInterval(interval);
  }, [match, location, history, startReplay]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (gameState) {
        setCurrentTick(gameState.getCurrentTick());
      }
    }, 1000);
  ...
  useEffect(() => {
    if (!gameFinished && game && game.finished) {
      if (finishSound) {
        finishSound.volume = soundVolume / 100;
        finishSound.play();
      }
    }
  }, [gameFinished, game, finishSound, soundVolume]);

  const initGameState = (gameId) => {
    const gameState = new GameState(gameId, undefined, fetchUserInfo, () => {});
    setGameState(gameState);

    gameState.registerListener((updatedGameState, updates) => {
      setGameState(updatedGameState);
      if (updates) {
        setGameFinished(
          updatedGameState.game ? updatedGameState.game.finished : gameFinished
        );
    ...
    <div className="game">
        ...
        <div>
          <audio
            ref={(musicRef) => {
              setMusic(musicRef);
              if (musicRef) {
                musicRef.volume = musicVolume / 100;
              }
            }}
            loop
          >
          ...
          {captureSounds.map((_, index) => (
            <audio
              key={index}
              ref={(soundRef) => (captureSounds[index] = soundRef)}
            >
              <source
      ...
      {isPortrait && (
        <div className="game-portrait-warning">
          Your browser appears to be in portrait mode. Rotate for a better
          experience!
        </div>
      )}
    </div>
...
//       musicVolume:
//         (window.localStorage && window.localStorage.musicVolume) || 0,
//       soundVolume:
//         (window.localStorage && window.localStorage.soundVolume) || 0,
//     };
...
//                 {Time.renderGameTime(currentTick) +
//                   " / " +
//                   Time.renderGameTime(totalTicks)}
//               </div>
